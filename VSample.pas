...
TYPE
  ...
  TVideoSample  = class(TObject)
                    private
                      ...
                      pIAMCameraControl : IAMCameraControl;
                    ...
                    public
                     ...
                     FUNCTION    GetVideoControlPropAmpEx(    Prop     : TCameraControlProperty;
                                                    VAR pMin, pMax,
                                                        pSteppingDelta,
                                                        pDefault       : longint;
                                                    VAR pCapsFlags     : TCameraControlFlags;
                                                    VAR pActual        : longint): HResult;
                      ...
                      FUNCTION    SetVideoControlPropAmpEx(    Prop    : TCameraControlProperty;
                                                        pCapsFlags     : TCameraControlFlags;
                                                        pActual        : longint): HResult;

                      PROCEDURE   GetVideoPropAmpPercent(Prop: TVideoProcAmpProperty ; VAR AcPerCent: integer);
                      PROCEDURE   GetVideoControlPropAmpPercent(Prop: TCameraControlProperty; VAR AcPerCent: integer);

                      PROCEDURE   SetVideoPropAmpPercent(Prop: TVideoProcAmpProperty; AcPerCent: integer);
                      PROCEDURE   SetVideoControlPropAmpPercent(Prop: TCameraControlProperty; AcPerCent: integer);
...
BEGIN
 ...
 //pIAMCameraControl
  if (pIAMCameraControl = nil) then
    if not(S_OK = pIBFVideoSource.QueryInterface(IID_IAMCameraControl, pIAMCameraControl)) then
      pIAMCameraControl := nil;
...
begin
  ...
  ShowPropertyDialogEx(pIBFVideoSource, FilterInfo.achName);
...
FUNCTION TVideoSample.GetVideoControlPropAmpEx(    Prop              : TCameraControlProperty;
                                        VAR pMin, pMax,
                                            pSteppingDelta, pDefault : longint;
                                        VAR pCapsFlags               : TCameraControlFlags;
                                        VAR pActual                  : longint): HResult;
BEGIN
  ...
  if pIAMCameraControl = nil then
    exit;
  //Result := pIAMCameraControl.GetRange(Prop, pMin, pMax, pSteppingDelta, pDefault, pCapsFlags);
  Result := pIAMCameraControl.GetRange(Prop, pMin, pMax, pSteppingDelta, pDefault, pCapsFlags);

  pActual := pDefault;
  IF Result = S_OK then
    Result := pIAMCameraControl.Get(Prop, pActual, pCapsFlags)
END;
...
FUNCTION TVideoSample.SetVideoControlPropAmpEx(    Prop           : TCameraControlProperty;
                                            pCapsFlags     : TCameraControlFlags;
                                            pActual        : longint): HResult;
BEGIN
  Result := S_False;
  if pIAMCameraControl = nil then
    exit;
  Result := pIAMCameraControl.Set_(Prop, pActual, pCapsFlags)
END;
...
PROCEDURE TVideoSample.GetVideoControlPropAmpPercent(Prop: TCameraControlProperty; VAR AcPerCent: integer);
VAR
  pMin, pMax,
  pSteppingDelta,
  pDefault       : longint;
  ...
  pActual        : longint;
BEGIN
  IF GetVideoControlPropAmpEx(Prop, pMin, pMax, pSteppingDelta, pDefault, pCapsFlags, pActual) = S_OK
    THEN BEGIN
      AcPerCent := round(100 * (pActual-pMin)/(pMax-pMin));
    END
    ELSE AcPerCent := -1;
...
PROCEDURE TVideoSample.SetVideoControlPropAmpPercent(Prop: TCameraControlProperty; AcPerCent: integer);
VAR
  pMin, pMax,
  pSteppingDelta,
  pDefault        : longint;
...
BEGIN
  IF GetVideoControlPropAmpEx(Prop, pMin, pMax, pSteppingDelta, pDefault, pCapsFlags, pActual) = S_OK
    THEN BEGIN
      ...
      pIAMCameraControl.Set_(Prop, pActual, pCapsFlags);
    END
END;
