...
TYPE
  ...
  TVideoImage = class
                  private
                    ...
                    function      TranslateControlProperty(const VCP: TVideoControlProperty; VAR CCP: TCameraControlProperty): HResult;
                  ...
                  public
                    ...
                    FUNCTION      GetVideoControlPropertySettings(    VCP                : TVideoControlProperty;
                                                           VAR MinVal, MaxVal,
                                                               StepSize, Default,
                                                               Actual            : integer;
                                                           VAR AutoMode: boolean): HResult;
                    FUNCTION      SetVideoPropertySettings(VP: TVideoProperty; Actual: integer; AutoMode: boolean): HResult;
                    FUNCTION      SetVideoControlPropertySettings(VCP: TVideoControlProperty; Actual: integer; AutoMode: boolean): HResult;
                end;



FUNCTION GetVideoPropertyName(VP: TVideoProperty): string;
FUNCTION GetVideoControlPropertyName(VCP: TVideoControlProperty): string;
...
BEGIN
  CASE VCP OF
    VCP_Pan                  : Result := 'Pan';
    VCP_Tilt                 : Result := 'Tilt';
    VCP_Roll                 : Result := 'Roll';
    VCP_Zoom                 : Result := 'Zoom';
    VCP_Exposure             : Result := 'Exposure';
    VCP_Iris                 : Result := 'Iris';
    VCP_Focus                : Result := 'Focus';
  END; {case}
END;
...
function TVideoImage.TranslateControlProperty(const VCP: TVideoControlProperty; VAR CCP: TCameraControlProperty): HResult;
begin
  Result := S_OK;
  CASE VCP OF
    VCP_Pan                  : CCP := CameraControl_Pan;
    VCP_Tilt                 : CCP := CameraControl_Tilt;
    VCP_Roll                 : CCP := CameraControl_Roll;
    VCP_Zoom                 : CCP := CameraControl_Zoom;
    VCP_Exposure             : CCP := CameraControl_Exposure;
    VCP_Iris                 : CCP := CameraControl_Iris;
    VCP_Focus                : CCP := CameraControl_Focus;
    else Result := S_False;
  END; {case}
end;

FUNCTION TVideoImage.GetVideoControlPropertySettings(VCP: TVideoControlProperty; VAR MinVal, MaxVal, StepSize, Default, Actual: integer; VAR AutoMode: boolean): HResult;
VAR
  CCP     : TCameraControlProperty;
  CCFlags : TCameraControlFlags;
BEGIN
  Result   := S_FALSE;
  MinVal   := -1;
  MaxVal   := -1;
  ...
  IF not(assigned(VideoSample)) or Failed(TranslateControlProperty(VCP, CCP)) then
    exit;

  Result := TranslateControlProperty(VCP, CCP);
  IF Failed(Result) then
    exit;

  Result := VideoSample.GetVideoControlPropAmpEx(CCP, MinVal, MaxVal, StepSize, Default, CCFlags, Actual);
  IF Failed(Result) then
    begin
      MinVal   := -1;
      MaxVal   := -1;
...
FUNCTION TVideoImage.SetVideoControlPropertySettings(VCP: TVideoControlProperty; Actual: integer; AutoMode: boolean): HResult;
VAR
  CCP     : TCameraControlProperty;
  CCFlags : TCameraControlFlags;
BEGIN
  ...
  IF not(assigned(VideoSample)) or Failed(Result) then
    exit;
  IF AutoMode
    then CCFlags := CameraControl_Flags_Auto
    else CCFlags := CameraControl_Flags_Manual;
  Result := VideoSample.SetVideoControlPropAmpEx(CCP, CCFlags, Actual);
END;
